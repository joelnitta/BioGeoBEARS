...
mapply_likelihoods_prebyte <- function(Qmat, phy2, transpose_needed)
	...
	# Use furrr::plan() to set number of cores used for parallel processing (not parallel by defualt)
	independent_likelihoods_on_each_branch = abind::abind(
		furrr::future_map2(list(Qmat), phy2$edge.length, ~rexpokit::expokit_dgpadm_Qmat(Qmat = .x, t = .y, transpose_needed=TRUE)),
		along = 3)
